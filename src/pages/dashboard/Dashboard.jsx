...
const AlertsSection = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      setLoading(true);
      try {
        const res = await axiosInstance.get(API_ENDPOINTS.DASHBOARD.ALERTS, {
          params: filters,
        });
...
const KPIRow = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchOverview = async () => {
      try {
        const res = await axiosInstance.get(API_ENDPOINTS.DASHBOARD.OVERVIEW, {
          params: filters,
        });
        if (res.data?.success) setData(res.data.data);
      } catch (err) {
    ...
    const fetchTrend = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          API_ENDPOINTS.DASHBOARD.PLAN_VS_ACTUAL,
          { params: filters },
        );
        if (res.data?.success) {
...
const CostSummary = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchCost = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          API_ENDPOINTS.DASHBOARD.COST_SUMMARY,
          { params: filters },
        );
        if (res.data?.success) setData(res.data.data);
      } catch (err) {
  ...
  const hasReworkPercent =
    data?.reworkPercent !== null && data?.reworkPercent !== undefined;
  const hasReworkedQty =
    data?.reworkedQty !== null && data?.reworkedQty !== undefined;
      ...
      <div className="dashboard-card-content quality-summary-content">
        <div className="quality-summary-main">
          <div className="quality-yield-panel">
            ...
            <strong className="quality-yield-value">
              {firstPassYield.toFixed(2)}%
            </strong>
            <progress
              className="quality-progress-native"
              value={Math.min(Math.max(firstPassYield, 0), 100)}
              max="100"
              aria-label={`First pass yield ${firstPassYield.toFixed(2)} percent`}
            />
          ...
          <div className="quality-rate-grid">
            ...
            <div className="quality-rate-tile is-warning">
              ...
              <strong>
                {hasReworkPercent
                  ? `${Number(data.reworkPercent).toFixed(2)}%`
                  : "—"}
              </strong>
            </div>
          </div>
        ...
        <div className="quality-quantity-strip">
          <div>
            ...
            <strong className="text-danger">
              {formatNumberCompact(data?.rejectedQty)}
            </strong>
          </div>
          <div>
            <span>Reworked Qty</span>
            <strong>
              {hasReworkedQty ? formatNumberCompact(data.reworkedQty) : "—"}
            </strong>
          </div>
          <div>
            <span>Total Produced</span>
            <strong>{formatNumberCompact(data?.totalProduced)}</strong>
          </div>
        </div>
...
const TopOrdersCostVariance = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchVar = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          API_ENDPOINTS.DASHBOARD.COST_VARIANCE,
          { params: filters },
        );
        if (res.data?.success) setData(res.data.data);
      } catch (err) {
  ...
  const varianceColumns = [
    {
      ...
      header: "Order #",
      width: "14%",
      render: (row) => <strong>{row.OrderNum}</strong>,
    },
    { key: "Product", header: "Product", width: "20%" },
    {
      key: "ActualCost",
      header: "Actual Cost / Unit",
      width: "18%",
      align: "right",
      render: (row) => formatNumberCompact(row.ActualCost),
    },
    {
      key: "StdCost",
      header: "Std Cost / Unit",
      width: "18%",
      align: "right",
      render: (row) => formatNumberCompact(row.StdCost),
    },
    {
      key: "VariancePercent",
      header: "Variance %",
      ...
      render: (row) => (
        <strong
          className={row.VariancePercent > 0 ? "text-danger" : "text-success"}
        >
          {row.VariancePercent}%
        </strong>
      ),
  ...
  const completionRate =
    data.totalOrders > 0 ? (data.completed / data.totalOrders) * 100 : 0;
  const orderStatuses = [
    ...
    <div className="dashboard-card chart-card-container dome-card-wrapper cost-variance-card">
      ...
      <div className="dashboard-card-content orders-summary-content">
        <div className="orders-overview">
          ...
          <span className="orders-completion-badge">
        ...
        <div className="orders-status-grid">
          {orderStatuses.map((status) => (
            <div
              key={status.label}
              className={`orders-status-tile is-${status.tone}`}
            >
...
const CriticalMaterialShortages = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchShortages = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          API_ENDPOINTS.DASHBOARD.MATERIAL_SHORTAGES,
          { params: filters },
        );
        if (res.data?.success) setData(res.data.data);
      } catch (err) {
  ...
  const shortageColumns = [
    { key: "ItemCode", header: "Material Code" },
    {
      ...
      width: "220px",
      render: (row) => <ExpandableText text={row.ItemName} maxLength={28} />,
    },
    {
      key: "RequiredQty",
      header: "Required Qty",
      align: "right",
      render: (row) => row.RequiredQty?.toLocaleString(),
    },
    {
      key: "AvailableQty",
      header: "Available Qty",
      align: "right",
      render: (row) => row.AvailableQty?.toLocaleString(),
    },
    {
      key: "Shortage",
      header: "Shortage",
    ...
    <div className="dashboard-card chart-card-container dome-card-wrapper shortage-summary-card">
      <div className="dashboard-card-header d-flex justify-between">
        ...
        <div className="summary-header-actions">
          {!loading && (
            <span className="summary-count-badge is-danger">
              {data.length} items
            </span>
          )}
        </div>
                  ...
                  {data.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="text-right text-danger">
                        <strong>{item.Shortage?.toLocaleString()}</strong>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
const DailyEfficiencyTrend = ({ filters }) => {
  ...
  const fetchEfficiency = async (page = 1, size = 10) => {
    ...
    try {
      ...
      const res = await axiosInstance.get(API_ENDPOINTS.DASHBOARD.EFFICIENCY, {
        params,
      });
      if (res.data?.success) {
  ...
  const columns = [
    { header: "Date", key: "date", width: "24%" },
    {
      ...
      width: "25%",
      align: "right",
      render: (row) => formatNumberCompact(row.plannedQty),
    },
    {
      ...
      width: "27%",
      align: "right",
      render: (row) => formatNumberCompact(row.completedQty),
    },
    {
      header: "Efficiency %",
      key: "efficiency",
      width: "24%",
      align: "right",
      render: (row) => `${row.efficiency?.toFixed(2) || "0.00"}%`,
    },
  ];
      ...
      <div className="dashboard-card-content efficiency-summary-content">
        <div className="chart-content w-100 chart-fixed-height efficiency-chart-panel">
          {loading && chartData.length === 0 ? (
            <div className="dashboard-state w-100">
              <IconRefresh className="spin" size={32} color="var(--primary)" />
            </div>
          ) : chartData.length > 0 ? (
            <LineChart
            ...
            <div className="d-flex align-center justify-center w-100 h-100 text-muted">
              No data available
            </div>
          )}
        </div>
...
const DowntimeSummary = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchDowntime = async () => {
      setLoading(true);
      try {
        const res = await axiosInstance.get(API_ENDPOINTS.DASHBOARD.DOWNTIME, {
          params: filters,
        });
  ...
  const totalDowntime = downtimeReasons.reduce(
    (sum, reason) => sum + (Number(reason.value) || 0),
    0,
  );
        ...
        <div className="summary-header-actions">
          {!loading && (
            <span className="summary-count-badge is-warning">
              {formatNumberCompact(totalDowntime)} hrs
            </span>
          )}
        </div>
      </div>
...
const OEEBreakdown = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchOEE = async () => {
      setLoading(true);
      try {
        const res = await axiosInstance.get(API_ENDPOINTS.DASHBOARD.OEE, {
          params: filters,
        });
        if (res.data?.success) setData(res.data.data);
      } catch (err) {
...
const QualitySummary = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchQuality = async () => {
      try {
        const res = await axiosInstance.get(API_ENDPOINTS.DASHBOARD.QUALITY, {
          params: filters,
        });
        if (res.data?.success) setData(res.data.data);
      } catch (err) {
      ...
      <div className="dashboard-card-content quality-summary-content">
        ...
        <div className="quality-summary-legacy">
          <div className="d-flex justify-between text-center gap-4 mb-4 legacy-quality-rates">
            ...
            <div className="flex-1 bg-light p-3 legacy-quality-tile">
              <p className="m-0 text-muted legacy-quality-label">
                First Pass Yield %
              </p>
...
const OrdersSummary = ({ filters }) => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await axiosInstance.get(
          API_ENDPOINTS.DASHBOARD.ORDER_SUMMARY,
          { params: filters },
        );
        if (res.data?.success) setData(res.data.data);
      } catch (err) {
  ...
  return (
    <div className="dashboard-card chart-card-container dome-card-wrapper summary-card orders-summary-card">
      ...
      <div className="dashboard-card-content orders-summary-content">
        ...
        <progress
          className=""
          value={Math.min(Math.max(completionRate, 0), 100)}
          max="100"
          aria-label={`${completionRate.toFixed(0)} percent of orders completed`}
        />
        <div className="orders-status-grid">
          {orderStatuses.map((status) => (
            <div
